<template>
  <div class="college-approvals">
    ...
    <div class="batch-bar" v-if="selectedRows.length > 0">
      ...
      <el-button type="success" size="small" style="margin-left: 12px" @click="handleBatchAction('APPROVE')">{{ $t("批量通过") }}</el-button>
      <el-button type="danger" size="small" style="margin-left: 8px" @click="handleBatchAction('REJECT')">{{ $t("批量拒绝") }}</el-button>
    </div>
    ...
    <el-dialog v-model="rejectVisible" title="拒绝报名" width="480px" destroy-on-close>
      <div class="reject-dialog-content">
        <p class="reject-info">{{ $t("申请人：") }}<strong>{{ currentReject?.userName }}</strong>
...
// ---- 活动选项（从数据中提取） ----
const activityOptions = ref<{ label: string; value: number }[]>{{ $t("([])
