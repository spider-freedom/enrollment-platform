<template>
  <div class="user-management">
    <div class="page-header"><h2>{{ $t("全校用户管理") }}</h2><p>{{ $t("管理全校所有学院的学生、教师和管理员账号") }}</p></div>
    ...
    <div class="stats-row">
      ...
      <div class="stat-card c2"><div class="stat-icon"><el-icon :size="22"><Star /></el-icon></div><div class="stat-value">{{ stats.schoolAdmins }}</div><div class="stat-label">{{ $t("学校管理员") }}</div></div>
      <div class="stat-card c3"><div class="stat-icon"><el-icon :size="22"><Avatar /></el-icon></div><div class="stat-value">{{ stats.collegeAdmins }}</div><div class="stat-label">{{ $t("学院管理员") }}</div></div>
      <div class="stat-card c4"><div class="stat-icon"><el-icon :size="22"><UserFilled /></el-icon></div><div class="stat-value">{{ stats.teachers }}</div><div class="stat-label">{{ $t("教师") }}</div></div>
      <div class="stat-card c5"><div class="stat-icon"><el-icon :size="22"><School /></el-icon></div><div class="stat-value">{{ stats.students }}</div><div class="stat-label">{{ $t("学生") }}</div></div>
    </div>
      ...
      <el-table :data="paginatedList" v-loading="loading" border stripe>
        ...
        <el-table-column label="角色" width="100">
          <template #default="{ row }">
            <el-tag v-if="isSchoolAdmin(row.role)" type="danger" size="small">{{ $t("学校管理员") }}</el-tag>
            <el-tag v-else-if="isCollegeAdmin(row.role)" type="warning" size="small">{{ $t("学院管理员") }}</el-tag>
        ...
        <el-table-column label="权限设置" width="100" fixed="right">
          <template #default="{ row }">
            <el-dropdown trigger="click" @command="(cmd: string) => handleCommand(cmd, row)">
              <el-button size="small" type="primary" link>{{ $t("设置 ▾") }}</el-button>
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item v-if="isTeacher(row.role)" command="promoteCollege">{{ $t("设为学院管理员") }}</el-dropdown-item>
                  <el-dropdown-item v-if="isTeacher(row.role)" command="promoteSchool">{{ $t("设为学校管理员") }}</el-dropdown-item>
                  <el-dropdown-item v-if="isCollegeAdmin(row.role)" command="promoteSchool">{{ $t("设为学校管理员") }}</el-dropdown-item>
                  <el-dropdown-item v-if="isCollegeAdmin(row.role) || isSchoolAdmin(row.role)" command="demote" divided>{{ $t("降为教师") }}</el-dropdown-item>
